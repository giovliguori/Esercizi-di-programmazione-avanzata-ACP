# Container_based_virtualization
...
By running the following command ``while True; do curl http://192.168.100.101:5001/; echo; sleep 1; done`` we make HTTP requests only towards the manager node (dockertest1: 192.168.100.101:5001). **NOTE: On some system ``true`` should be used in lieu of ``True``**.
...
To stop the service:
```
docker service rm flask_helloworld_service
```
